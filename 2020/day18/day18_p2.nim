...
proc evaluate(expression: string): int64 =
    ...
    if i == -1:
        var expression = expression.splitWhitespace().map(e => e.strip(true, true, {'(', ')'})).toSeq
        while "+" in expression:
            var j = 0
            var r = 0
            for i, e in expression:
                if e == "+":
                    r = expression[i-1].parseInt + expression[i+1].parseInt
                    j = i
                    break
            expression[j] = $r
            expression.delete(j-1)
            expression.delete(j)
